...
* [822](https://github.com/dbekaert/RAiDER/pull/822) - Height datums are now explicit. Heights previously flowed into weather-model sampling unconverted, silently treating every source as geoid; each source is now kept in its own datum -- ellipsoidal for GNSS positions and ISCE geometry rasters, geoid for GLO-30 DEMs -- and converted only where a consumer needs the other. **Behaviour changes for existing inputs:**
  * Two new `height_group` options name the datums explicitly: `height_datum` for the query points (a station file's `Hgt_m` column), defaulting to `ellipsoidal` since that is what GNSS positions carry; and `dem_height_datum` for any DEM whether downloaded or supplied via `dem`, defaulting to `geoid`, the native datum of GLO-30, SRTM and anything RAiDER fetches itself. Both accept `'geoid'` or `'ellipsoidal'`. An unlabelled `Hgt_m` column is now read as ellipsoidal and converted, so a file holding MSL heights must say `height_datum: geoid`. Note `write_yaml` merges only the top level of a config, so one supplying its own `height_group` must spell these keys out.
  ...
  * `StationFile` takes `height_datum` and `dem_height_datum`; `RasterRDR` and `GeocodedFile` take `dem_height_datum`; `readZ()` takes `geoid_heights: bool = False` across all three.
  * `LOS.setPoints()`'s third parameter is renamed `heights` -> `ellipsoidal_heights`. The old keyword still works for one release and emits a `DeprecationWarning`.
  ...
  * A cached DEM without a `RAIDER_HEIGHT_DATUM` tag is discarded and re-downloaded rather than reused. One written by an earlier RAiDER holds ellipsoidal heights, so reinterpreting it as geoid would move every height by the geoid undulation from an otherwise unchanged config. A DEM supplied through `dem` is never second-guessed this way -- it carries no tag by definition and its datum comes from `dem_height_datum`.
  ...
  * **Not fixed here:** the cube path still mixes datums. `_build_cube_ray` feeds the cube's z-levels to ECEF ray geometry as ellipsoidal heights, while the zenith path interpolates those same levels against the geoid-referenced weather model, and `calcDelaysGUNW` supplies them from GUNW `heightsMeta`. The `z` attribute is therefore written per path -- `height above geoid` for the zenith and projected cubes, `height above ellipsoid` for ray-traced ones -- rather than asserting one convention for both. Pre-existing, and tracked separately.

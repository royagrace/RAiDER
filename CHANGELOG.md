...
### Fixed
* [823](https://github.com/dbekaert/RAiDER/pull/823) - Stopped the test suite from writing derived weather models into the repository. `combine_weather_files` writes the time-interpolated model next to its inputs, so fixtures pointing at tracked directories overwrote two committed `_timeInterp_` files under `test/gunw_test_data` and rewrote ~165 MB of derived models into `test/gunw_azimuth_test_data` on every run. The derived filenames come from the GUNW scene's fixed center time, so each run overwrote the previous one rather than accumulating, and a blanket `.gitignore` entry kept all of it out of `git status`. The derived products are now untracked, the entries that concealed them have been removed so a regression shows up in `git status`, and the weather-file fixtures hand tests a scratch directory of symlinks so the output lands outside the repo. A session-level check fails the run if a weather model appears in a tracked fixture directory. Because `tmp_path_factory` hands each worker its own directory, the fixtures are also safe to run under `pytest-xdist`, where parallel workers previously raced to write one file in the repository; the session check itself is not xdist-aware and has not been exercised under `-n`.

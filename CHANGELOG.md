...
* [822](https://github.com/dbekaert/RAiDER/pull/822) - Height datums are now explicit. Heights previously flowed into weather-model sampling unconverted, silently treating every source as geoid; each source is now kept in its own datum -- ellipsoidal for GNSS positions and ISCE geometry rasters, geoid for GLO-30 DEMs -- and converted only where a consumer needs the other. **Behaviour changes for existing inputs:**
  ...
  * **Not fixed here:** the cube path still mixes datums. `_build_cube_ray` feeds the cube's z-levels to ECEF ray geometry as ellipsoidal heights, while the zenith path interpolates those same levels against the geoid-referenced weather model, and `calcDelaysGUNW` supplies them from GUNW `heightsMeta`. The `z` attribute this PR relabels `height above geoid` is therefore only accurate under the zenith reading. Pre-existing, and tracked separately.
